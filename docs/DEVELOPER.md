...
#### Logout redirect follow-up (BUG-AUTH-3b, 2026-06-02)

After BUG-AUTH-3b shipped, browser smoke surfaced a regression: the success toast fired but the browser stayed on the dashboard — the user was never redirected to `/login`. Root cause: `/logout` is a Next.js **Route Handler** (`src/app/logout/route.ts`, exports `GET`/`POST` returning `NextResponse.redirect(...)`), NOT a Page. The Next.js App Router `router.push()` is designed for Page routes — when handed a Route Handler target it attempts an RSC fetch that either errors silently or no-ops, so the toast fired but the browser never navigated.

- **Fix.** `handleLogout` in `src/components/dashboard/AuthDashboardShell.tsx` now calls `window.location.assign("/logout")` instead of `router.push("/logout")`. The browser issues a real GET to `/logout`, hits the GET handler, follows its 302 to `/login`, and reloads the document. This matches the documented intent of the `/logout` route's JSDoc (which shows `window.location.href = 'https://auth.xynes.com/logout?...'` as the canonical caller pattern from consumer apps).
- **Why not server-route the toast?** The `/logout` route is server-side — it can't fire a client-visible toast before it 302s. The toast must fire on the dashboard BEFORE the navigation starts, so the user sees feedback during the ~500 ms between click and `/login` paint. The success-toast-then-navigate ordering is preserved byte-for-byte.
- **Why `window.location.assign` over `window.location.href = "/logout"`?** Equivalent in browsers (both push a new history entry) but `.assign(url)` is a method we can spy on cleanly in jsdom tests. `.href = url` is an accessor assignment that jsdom intercepts with internal navigation throws.
- **Failure path still works.** The try/catch around `window.location.assign("/logout")` still fires the destructive error toast on a thrown navigation. In practice jsdom + real browsers don't throw on `window.location.assign` (the navigation just happens), but the defense-in-depth catch stays for environments that do (e.g. content-security-policy or iframe-sandbox restrictions).
- **Test wiring.** `AuthDashboardShell.integration.test.tsx` now stubs `window.location.assign` via `Object.defineProperty(window, "location", { configurable: true, writable: true, value: Object.assign({}, window.location, { assign: mockLocationAssign }) })` inside `beforeEach`. All four logout assertions migrated from `mockPush` to `mockLocationAssign`. A regression-guard assertion `expect(mockPush).not.toHaveBeenCalledWith("/logout")` was added inside the BUG-LDS-2 test so future refactors that revert to `router.push` fail loudly.
- **Quality gates (2026-06-02).** `pnpm lint` clean. `pnpm test src/components/dashboard/AuthDashboardShell` → 30/30 pass (3 files). Full suite `pnpm test` → 910/910 pass (87 files) — net zero new failures.
- **CMS Console parity — NOT in this fix.** `CmsDashboardShell.tsx` has the same `router.push("/logout?redirect=...")` pattern. The same fix likely applies there; this PR scopes only to `xynes-auth-app` because the bug report named the Auth App. Follow-up: file a parallel fix for CMS Console.
- **No DB / schema / migration / env-var impact.** Pure client-side wiring change.
...
## LP-AUTH — Public Landing Page at `/`

Landed 2026-06-04. Replaces the legacy `redirect("/login")` one-liner at `src/app/page.tsx` with a real public splash for first-time visitors to `https://auth.xynes.com/`.

**Ownership rules**

- The page lives in **this repo** (`xynes-front-end/xynes-auth-app`), not in the apex marketing site (`xynes-marketing-web`, still planned). The auth-app owns the visual surface every visitor hits when they deep-link or bookmark `auth.xynes.com`.
- Visible copy lives in `messages/en-US/auth.landing.json` (en-XA mirror auto-generated by `pnpm generate:pseudo`). The translator-context sidecar is at `messages/en-US/auth.landing.meta.json`.
- The human-editable source of truth for copy + URL structure is `docs/marketing-copy.md`. Non-engineers update copy via PR there, then the engineer mirrors the change into the JSON catalogs.
- Structural data (icon ids, internal/external URLs, footer column layout) lives in `src/lib/landing-copy.ts` — kept out of catalogs so translators cannot accidentally edit URLs.

**Architecture**

```
src/app/page.tsx                      ← RSC: server-side auth check + redirect-on-session
  ├─ delegates rendering to ↓
src/components/landing/LandingScreen  ← 'use client' (next-intl translations)
  ├─ imports from @lumia-ui/marketing ← MarketingNav, MarketingHero,
  │                                     MarketingFeatureGrid + MarketingFeatureCard,
  │                                     MarketingTrustStrip, MarketingFooter,
  │                                     CookieDisclosure
  ├─ imports from @lumia-ui/icons     ← Icon (shield-check, globe, code)
  └─ reads structural data from
     src/lib/landing-copy.ts
```

**Authenticated visitors**

When a Supabase session is present, the RSC short-circuits with `redirect(safeRedirect)`. The redirect target is either:

1. The validated `?redirect=<url>` query (passed through `getSafeRedirectUrl` against `getAllowedRedirectDomains()`), or
2. The default `/dashboard/apps`.

Hostile / unallowlisted / protocol-relative `?redirect=` values fail closed to `/dashboard/apps`. This matches the rest of the auth-app's redirect posture and prevents an open-redirect via the new page.

**Anonymous visitors**

Render `<LandingScreen>` with the validated `safeRedirect` forwarded as `signupRedirect`. The signup CTA composes `/signup?redirect=${encodeURIComponent(signupRedirect)}` so the original cross-app intent survives the handshake.

**`/SECURITY.md`**

The trust strip's "Security policy" chip points at `/SECURITY.md`. The file lives at TWO paths:

1. **`SECURITY.md`** (repo root) — picked up by GitHub's security-policy detector.
2. **`public/SECURITY.md`** — served by Next.js as a static file.

`src/lib/landing-copy.test.ts` asserts the two copies are byte-identical so they cannot silently drift.

**No third-party analytics**

The page mounts ZERO browser analytics, ZERO `posthog-js`, ZERO Google Analytics. STORAGE-LIVE-5's gateway-architecture feature-flag pattern is server-side only and is not consumed here.

**Cookie disclosure**

Non-blocking, non-modal sticky disclosure at the bottom of the viewport (Lumia DS `<CookieDisclosure>`). Dismissal persists in `localStorage`, NOT in a cookie. This matches LP-DS / LP-AUTH §6 invariants.

**Allowed redirect domains**

The page consumes the existing repo-local helpers `getAllowedRedirectDomains()` and `getSafeRedirectUrl()` from `src/lib/redirect`. No new auth/redirect helpers are introduced — the contract is exactly the same as `/login`, `/signup`, `/workspaces`.

**Public-routes contract**

The middleware in `src/middleware.ts` does NOT enforce auth on `/`. The page handles its own auth check at the RSC level. This mirrors `/login` and `/signup` — both are public, both do their own already-authenticated redirect.

**Docker dev wiring**

Adding the `@lumia-ui/marketing` package needs a parallel `xynes-front-end/infra` change (Dockerfile + compose mounts). Tracked on branch `feature/LP-AUTH-docker-mounts-marketing`. Without that branch, `pnpm dev` in `xynes-front-end/infra` would not find the marketing package inside the container until the image is rebuilt.

**Container rebuild required after merging LP-AUTH.** The `xynes-auth-app` service does not mount `package.json` from host. After merging both LP-AUTH branches, the operator MUST rebuild the dev image (`docker compose restart` is NOT sufficient — `package.json` is COPY-ed in the Dockerfile, not mounted):

```bash
cd xynes-front-end/infra
docker compose -f docker-compose.dev.yml build xynes-auth-app
docker compose -f docker-compose.dev.yml up -d --force-recreate xynes-auth-app
```

Verify with `docker exec xynes-auth-app ls /app/xynes-auth-app/node_modules/@lumia-ui/` — output must include `marketing`. The repo memory note at `/memories/repo/lp-auth-docker-rebuild-trap.md` documents the recovery sequence; same trap previously hit `drizzle-orm` (STORAGE-LIVE-3).

**Next.js `transpilePackages`.** `@lumia-ui/marketing` (and `@lumia-ui/layout`) must be listed in `transpilePackages` in `next.config.mjs` for webpack to follow the `link:` symlink. Without it, Next.js 15 + webpack fails resolution with `Module not found: Can't resolve '@lumia-ui/marketing'`.

**Test coverage**

- `src/lib/landing-copy.test.ts` — 14 tests covering structural data (frozen invariants, safe URL guard, OSS allowlist), catalog parity (en-US ↔ en-XA), no-leaks sweep (hostile substrings like `xynes_live_`, `AKIA*`, `X-Amz-Signature`), `buildFooterColumns()` behaviour including the hostile-translator regression guard, and the `SECURITY.md` byte-identity check.
- `src/components/landing/LandingScreen.test.tsx` — 10 tests covering the single-`<h1>` invariant, the three feature cards, hero CTAs as anchors with safe URLs + encoded redirect, external links with `target="_blank rel=noopener noreferrer"`, the trust strip chips, the four footer columns + copyright, the no-`<form>` invariant, and the cookie disclosure policy URL.
- `src/app/page.test.tsx` — 10 tests covering the RSC auth-check branch (logged-in → redirect to validated target; anonymous → render LandingScreen), hostile `?redirect=` (`javascript:`, protocol-relative, unallowed external host) all failing closed to the default destination.
- Net delta: +34 tests across 3 new files. Full suite **955/955 pass** (was 921). Overall coverage **91.35% stmts / 83.13% branches / 90.47% funcs / 91.35% lines** (above ADR-001 80% floor). Per-touched-file: `src/lib/landing-copy.ts` 100% / 100% / 100% / 100%; `src/components/landing/LandingScreen.tsx` 100% / 100% / 100% / 100%.

**Out of scope (deferred, per LP-AUTH §14)**

- Workspace-branded landing pages.
- Cookie consent modal (we use the non-blocking disclosure).
